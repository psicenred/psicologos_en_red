...
const COURSE_ID_RE =
  /^\/cursos\/alumno\/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})(?:\/|$)/i;
...
export function AlumnoSidebar({
  ...
  const globalNav: NavItem[] = [
    { href: '/cursos/alumno', label: 'Mis cursos', icon: LayoutDashboard, exact: true },
    { href: '/cursos/alumno/perfil', label: 'Editar mi perfil', icon: UserCircle },
  ];

  const courseNav: NavItem[] = useMemo(() => {
    if (!courseId) return [];
    const base = `/cursos/alumno/${courseId}`;
    const items: NavItem[] = [
      ...
      { href: `${base}/examenes`, label: 'Exámenes', icon: FileCheck },
      { href: `${base}/calificaciones`, label: 'Calificaciones', icon: Award },
      { href: `${base}/foro`, label: 'Foro', icon: MessageSquare },
    ];
    if (course?.format === 'sync') {
      items.push({ href: `${base}/en-vivo`, label: 'En vivo', icon: Video });
    }
    items.push({ href: `${base}/certificado`, label: 'Certificado', icon: BadgeCheck });
    return items;
    ...
    <aside className="flex w-full shrink-0 flex-col bg-secondary text-secondary-foreground shadow-lg md:w-[280px] md:min-h-screen">
      <div className="flex items-center gap-4 border-b border-white/15 px-5 py-4 md:flex-col md:items-stretch md:gap-0 md:py-4">
        <div className="flex-1 md:flex-none">
          <Link href="/academia" className="block text-xs font-semibold uppercase tracking-wider text-white/70">
            Academia
          </Link>
          <p className="mt-0.5 text-sm font-bold text-white">Psicólogos en Red</p>
        </div>
        <div className="flex items-center gap-3 md:hidden">
          <StudentAvatar name={studentName} avatarUrl={avatarUrl} size="sm" />
      ...
      <div className="hidden border-b border-white/15 px-5 py-6 text-center md:block">
        <div className="mx-auto mb-3 flex justify-center">
          <StudentAvatar name={studentName} avatarUrl={avatarUrl} size="lg" />
        </div>
        <p className="truncate text-base font-semibold text-white">{studentName}</p>
        <p className="mt-0.5 truncate text-xs text-white/70">{studentEmail}</p>
      </div>
        ...
        {!courseId ? (
          ...
          <>
            ...
            <div className="mt-4 flex gap-1 border-t border-white/15 px-2 pt-4 md:block md:space-y-0.5">
              <NavLink
                item={{ href: '/cursos/alumno/perfil', label: 'Editar mi perfil', icon: UserCircle }}
                active={normalized.startsWith('/cursos/alumno/perfil')}
              />
            </div>
          </>
        )}
